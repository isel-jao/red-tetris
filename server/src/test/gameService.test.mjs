...
describe("Game Service", () => {
  ...
  describe("createGame", () => {
    ...
    it("should throw an error when creating a game with an existing ID", () => {
      ...
      expect(() => gameService.createGame(gameId, "player-2")).toThrow(
        `Game with ID ${gameId} already exists`
      );
    });

    it("should throw an error when gameId or hostPlayerId is missing", () => {
      expect(() => gameService.createGame("", "player-1")).toThrow(
        "Game ID and host player ID are required"
      );

      expect(() => gameService.createGame("test-game", "")).toThrow(
        "Game ID and host player ID are required"
      );
    });
  });
  ...
  describe("joinGame", () => {
    ...
    it("should throw an error when joining a non-existent game", () => {
      expect(() =>
        gameService.joinGame("non-existent-game", "player-1")
    ...
    it("should throw an error when player is already in the game", () => {
      ...
      expect(() => gameService.joinGame(gameId, playerId)).toThrow(
